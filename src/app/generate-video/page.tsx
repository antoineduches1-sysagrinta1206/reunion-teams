...
export default function ScenarioBuilder() {
  ...
  const handleGenerateAll = useCallback(async () => {
    ...
    try {
      ...
      const VIDEO_PROMPT = 'Continuous uninterrupted webcam shot, head and shoulders framing, fixed camera with no movement. A person is in a live professional video conference call. The person exhibits constant subtle natural micro-movements throughout the entire video: gentle slow weight shifts, small natural head tilts and nods, occasional eyebrow raises, slow relaxed blinking at varied intervals, subtle chest breathing motion, and minor natural postural adjustments — all flowing smoothly into one another so the video never appears to loop or repeat. During silent sections of audio the person listens attentively with their mouth fully closed and still — no lip or jaw movement at all — but they continue to subtly shift, blink, breathe, and make small reactive expressions as if listening to someone speak (slight nods, faint smiles, brief eyebrow movements). When speech audio begins, the person speaks with precise natural lip sync and natural conversational head motion, blending seamlessly from the idle listening state. Transitions between speaking and listening must be smooth and gradual with no abrupt changes. Photorealistic webcam quality, soft natural office lighting, shallow depth of field on the background.'
